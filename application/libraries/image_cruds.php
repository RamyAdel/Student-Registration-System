...
class image_CRUDs {
	...
	protected function _upload_file($upload_dir) {
		...
		if (!empty($upload_response)) {
			...
			{
				$ci->image_moos->load($path)->resize(1024,768)->save($path,true);
			}
			/* ------------------------------------- */		
    ...
    protected function _insert_table($file_name, $relation_id = null)
    {
    	...
    	$this->ci->db->where('id', $relation_id);
    	$this->ci->db->update($this->table_name, $insert);
	...
	function render()
	{
		...
		$ci->load->helper('url');
		$ci->load->library('Image_moos');
		$this->image_moos = new Image_moos();
